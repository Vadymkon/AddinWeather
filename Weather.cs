...
namespace TestAddinWeather
{
    ...
    {
        private readonly string apiKey = "dcccfefbd932d021573b9a94645ac4f5"; // Replace with your weather API key
        private readonly string apiKeyCrypto = "CF4415CB-E39E-4B31-94D1-732434BC56CB"; // Replace with your cryptocurrency API key
        ...
        public async Task<JObject> GetCryptoDataAsync()
        {
            ...
            return JObject.Parse(result); // Parse the JSON response
        }
        ...
        public async Task<JArray> GetWeatherDataAsync(double lat, double lon)
        {
            ...
            request.AddParameter("units", "metric"); // Units in Celsius

            var response = await client.ExecuteAsync(request);
            string result = response.Content;

            var json = JObject.Parse(result);
            return (JArray)json["list"]; // Retrieve the "list" array with weather data
        }
        ...
        public void InsertWeatherDataToExcel(Excel.Worksheet worksheet, JArray weatherData)
        {
            ...
            if (startRow > 2)
            {
                ...
                usedRange.AutoFilter(1); // Apply filter to the used range
            }
            else
            {
        ...
        private async void button1_Click(object sender, RibbonControlEventArgs e)
        {
            ...
            {
                double latitude = 50.4501; // For example, Kyiv
